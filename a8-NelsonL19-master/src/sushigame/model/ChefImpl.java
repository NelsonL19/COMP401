...
public class ChefImpl implements Chef, BeltObserver {
	...
	private double totalFoodSpoiled;
	...
	public void handleBeltEvent(BeltEvent e) {
		...
		if (e.getType() == BeltEvent.EventType.PLATE_CONSUMED) {
			...
			if (plate.getChef() == this) {
				...
				IngredientPortion[] ingres = plate.getContents().getIngredients();
				
				for (int i = 0; i < ingres.length; i++) {
					amount += ingres[i].getAmount();
				}
	...
	@Override
	public double totalFoodSpoiled() {
		return totalFoodSpoiled;
	}
}
